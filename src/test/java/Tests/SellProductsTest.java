...
public class SellProductsTest extends BaseTest {
    @BeforeMethod
    public void setUp() {
        baseSetUp("CHROME", "112");
    }

    @Test(description = "User sold product successfully")
    @Description("User sold any product successfully after registered demo account")
    @Parameters({"countryCodeNumber", "symbol", "symbolValue"})
    public void sellProduct(String countryCodeNumber, String symbol, String symbolValue) throws IOException {
        ...
        wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//div[@symbol='"+symbol+"']"+
                "/div[6]/div[1]")));
        WebElement product = driver.findElement(By.xpath("//div[@symbol='"+symbol+"']"+"/div[6]/div[1]"));

        productsPage.clickElement(product, " " + symbol + " product");
        productsPage.selectElement(productsPage.amountField, "10,000", "lots");
        productsPage.clickElement(productsPage.sellButton, " sell now button");
        ...
        wdWait.until(ExpectedConditions.visibilityOf(driver.findElement(By.xpath("//span[contains(@class," +
                "'webSymbol') and text()='"+symbolValue+"']"))));
        wdWait.until(ExpectedConditions.elementToBeClickable(By.xpath("//span[contains(@class,"+"" +
                "'webSymbol') and text()='"+symbolValue+"']")));
